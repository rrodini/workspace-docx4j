...
class TestReferendumExtractor {
	...
	static void setUpBeforeClass() throws Exception {
		Initialize.referendumRegex = Utils.compileRegex("^(?<question>((.*\n).*REFERENDUM.*\n))(?<text>(.*\n)*?)^YES$\nNO");
	}
	...
	void testExtractReferendums1() {
		List<Referendum> refs = ReferendumExtractor.extractReferendums("025", "WEST_BRADFORD_1", page2Text);
		assertEquals(1, refs.size());
	}

	@Test
	void testExtractReferendums2() {
		List<Referendum> refs = ReferendumExtractor.extractReferendums("025", "WEST_BRADFORD_1", "");
	...
	@Test
	void testExtractPageReferendums2() {
		Ballot ballot = new Ballot("025_WEST_BRADFORD_1", "");
		ballot.setPage1Text(page1Text);
		ballot.setPage2Text("");
		ReferendumExtractor.extractPageReferendums(ballot);
